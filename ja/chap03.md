...
### 02 対象アイテムと各アイテムとのユークリッド距離 | 辞書内包表記
対象アイテム7について、各アイテム$$j \in I_{u}$$とのユークリッド距離のペア`{j: dist(x[7], x[j])}`を要素とした辞書`dists`を作成しなさい。確認コードを実行したとき、実行結果のとおりの結果が出力されること。
...
### 03 各対象アイテムと各アイテムとのユークリッド距離 | 辞書のネスト
課題02で作成した辞書`dists`を、各対象アイテム$$i \in \overline{I}_{u}$$について作成し、`{i: dists}`を要素とした辞書`dists_dict`を作成しなさい。確認コードを実行したとき、実行結果のとおりの結果が出力されること。
...
### 04 対象アイテムの近傍アイテム集合 | 辞書のソート、辞書のキー
対象アイテム9の近傍アイテム集合$$I_{i}$$を`ndarray`のベクトル`Ii`として取得しなさい。
...
## 推薦
対象ユーザ$$u$$の対象アイテム$$i$$への予測評価値$$\hat{r}_{u,i}$$は次式により定義される。
$$
\hat{r}_{u,i} = 
 \begin{cases}
...
### 07 対象ユーザの対象アイテムに対する予測評価値 | 条件分岐
対象ユーザ$$u$$の対象アイテム$$i$$に対する予測評価値$$r_{u,i}$$を求めなさい。
