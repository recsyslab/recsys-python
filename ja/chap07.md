...
### 13 第d主成分までの固有ベクトル
第$$d$$主成分までの対応する固有ベクトルを列ベクトルとして並べた行列$$\boldsymbol{V}$$を`ndarray`として取得するコードを書きなさい。得られた`ndarray`を`V`とすること。
...
すべてのアイテムの次元削減後の特徴ベクトルを`ndarray`としてまとめて求めるコードを書きなさい。得られた`ndarray`を`x3`とすること。

コード
```python
# すべてのアイテムの次元削減後の特徴ベクトル
【問題15】
print('x\'\' = \n{}'.format(x3))
```
