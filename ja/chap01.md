...
## アイテム
`Du`の各行のインデックス`i`は各アイテムアイテムID $$i$$に対応する。評価履歴$$D_{u}$$に含まれるアイテム集合は$$I = \{0, 1, \ldots, 12\}$$となる。このとき、次の問いに答えなさい。

### 07 アイテム集合
アイテム集合$$I$$をアイテムIDを要素とした`ndarray`のベクトルとして生成するコードを書きなさい。得られた`ndarray`を`I`とすること。
