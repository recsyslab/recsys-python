...
### 05 評価履歴の全要素数
評価履歴$$D_{u}$$の全要素数を取得するコードを書きなさい。ただし、欠損値も含む。
...
### 06 アイテム集合
`Du`の各行のインデックス`i`は各アイテム$$i$$のアイテムID $$i$$に対応する。アイテム集合$$I$$（アイテムIDを要素としたベクトル）を`ndarray`として生成するコードを書きなさい。得られた`ndarray`を`I`とすること。
