...
### 07 評価値行列の全要素数
評価値行列$$\boldsymbol{R}$$の全要素数を取得するコードを書きなさい。ただし、欠損値も含む。
...
### 08 観測されているか否かの判定
評価値行列$$\boldsymbol{R}$$において、観測値の要素には`True`を、欠損値の要素には`False`を入れたブール値配列を生成するコードを書きなさい。
...
### 09 評価値行列の観測値数
評価値行列$$\boldsymbol{R}$$における観測値数$$\mid R \mid$$を取得するコードを書きなさい。
