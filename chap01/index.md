...
`Du`からアイテム集合（各行のインデックスを要素としたベクトル）を`ndarray`として生成しなさい。生成したベクトルを`I`とすること。

★★
1. `np.arange()`を使う。
2. `ndarray.shape`を使う。

## 09 評価履歴からアイテムの特徴ベクトル集合の取得 | 行列のスライス
`Du`からアイテムの特徴ベクトル集合（最後の列を除くいた行列）を取得しなさい。取得した行列を`x`とすること。

★★
1. 行列のスライスを使う。

## 10 i番目の特徴ベクトルの取得 | 行列のインデックス参照
`x`から`0`行目の特徴ベクトルを取得しなさい。
...
## 評価値集合から部分集合の取得 | ベクトルのスライス
`ru`の`2`番目から`4`番目の要素を取得しなさい。

★
1. ベクトルのスライスを使う。

## 評価値集合の要素を逆順に取得 | ベクトルのスライス（逆順）
`ru`の要素を逆順に取得しなさい。
